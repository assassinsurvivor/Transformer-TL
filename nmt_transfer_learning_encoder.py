...
class Transformer_train(tf.keras.Model):
    ...
    def call(self,inp,tar,padded_mask,look_ahead_mask,is_train,pretrained_encoder):
        encoder_output=self.encoder(inp,padded_mask,is_train)
        encoder_output=tf.keras.layers.LayerNormalization()(encoder_output+pretrained_encoder)
        decoder_output,attn_weight_dict=self.decoder(tar,encoder_output,look_ahead_mask,padded_mask,is_train)
        final_output=self.final_layer(decoder_output)
        return final_output,attn_weight_dict,encoder_output
...
def loss_acc_metric():
    return tf.keras.metrics.Mean(name='train_loss'),tf.keras.metrics.SparseCategoricalAccuracy(name='train_accuracy'),tf.keras.metrics.Mean(name='valid_loss'),tf.keras.metrics.SparseCategoricalAccuracy(name='valid_accuracy')


train_loss,train_accuracy,valid_loss,valid_accuracy=loss_acc_metric()
